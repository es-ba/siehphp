...
if new.cla_ope = 'eah2013' then
  IF new.cla_for='TEM' and new.cla_mat='' THEN
    ...
    pla_id_marco,pla_comuna,pla_replica,pla_up,pla_lote,pla_clado,pla_ccodigo,pla_cnombre,pla_hn,pla_hp,pla_hd, pla_hab,pla_h4,pla_usp,pla_barrio,pla_ident_edif,pla_obs,pla_frac_comun,pla_radio_comu,pla_mza_comuna,
    pla_marco,pla_titular,pla_zona,pla_para_asignar,pla_participacion, pla_tlg, pla_dominio, pla_lote2011, pla_codpos, pla_etiquetas,pla_tipounidad,pla_tot_hab,pla_estrato ) 
    (SELECT $3, $4, $5, $6,  tem_id_marco,tem_comuna,tem_replica,tem_up,tem_lote,tem_clado,tem_ccodigo,tem_cnombre,tem_hn,tem_hp,
    tem_hd,tem_hab,tem_h4,tem_usp,tem_barrio,tem_ident_edif,tem_obs,tem_frac_comun,tem_radio_comu,tem_mza_comuna,tem_marco,tem_titular,tem_zona,tem_para_asignar,tem_participacion, $2, tem_dominio, tem_lote2011, tem_codpos,
    tem_etiquetas,tem_tipounidad,tem_tot_hab,tem_estrato FROM encu.tem  WHERE tem_enc=$1)';
