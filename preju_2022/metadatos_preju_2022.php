...
class Metadatos_preju_2022 extends Objeto_de_la_base{ 
    ...
    function ejecutar_instalacion($con_dependientes=true){
        ...
        if($via_json){
            $separador=";/*FIN*/\n";
            $json=json_decode(json_arreglar(file_get_contents('..\operaciones_preju_2022\preju_2022_dump.json')),true);
            if(!$json){
                throw new Exception_Tedede("Error al parsear el json preju_2022_dump.json. Error ".json_str_error());
            }
            $sentencias="";
            ...
            $separador=";\n";
            $sentencias=file_get_contents('..\operaciones_preju_2022\migraciones\metadatos.sql');
        }
    ...
    function instalar_tem_de_prueba(){
        ...
        if($cuantos=$tabla_tem->contar_cuantos(array())){ // CORRECTO UN = DE ASIGNACIÓN
            ...
            if($GLOBALS['NOMBRE_APP']=='preju_2022'){
            ...
            $this->contexto->db->ejecutar_sql(new Sql("INSERT INTO claves (cla_ope, cla_for, cla_mat, cla_enc, cla_tlg)   SELECT 'preju_2022','TEM','',tem_enc,tem_tlg     FROM tem; "));
        }
    }
