...
require_once "metadatos_eah2012.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../eah2012");

class Aplicacion_eah2012 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EAH2012";
        }else{
            $this->salida->html_title="TEST - eah2012";
        }
        ...
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="eah2012.manifest";
            }
        }
...
JS
        ...
        //probar_todo($este,array('../encuesta/casos_prueba_encuesta.js')); // OJO comentareado para poder crear jsones
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array('insercion_tabla_tem.sql',
                                  ...
                                  '..\operaciones_eah2012\insercion_tabla_personal.sql',
                                  //'..\operaciones_eah2012\insertar_estados_ingreso_consistencias.sql',
                                  'insercion_tabla_consistencias.sql',  
                                  'vista_var_orden.sql',
                                   ) as $archivo)
                    {
                        $este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'AJUS',"),array(','.PRIMER_TLG,"'eah2012',"),file_get_contents($archivo))));
                    }
                    $consistencias=explode("INSERT INTO consistencias",file_get_contents("../operaciones_eah2012/consistencias_eah2012.sql"));
                    ...
                    $estructura_encuesta = new Estructura_eah2012();
    ...
    function proceso_actualizar_instalacion(){
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                global $esta_es_la_base_en_produccion;
                try{
                    Aplicacion_eah2012::salida_enviar_aviso_instalacion($este);
                    ...
                    $este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_eah2012\actualizar_a_eah2012.sql'))));
                    $estructura_encuesta = new Estructura_eah2012();
                    $estructura_encuesta->contexto=$este;
                    $estructura_encuesta->generar_estructura();
                    if($esta_es_la_base_en_produccion){
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_eah2012");
    ...
    function proceso_desplegar_formulario($formulario='',$matriz=''){
        ...
        /* 
            http://localhost/yeah/eah2012/eah2012.php?hacer=desplegar_formulario&todo={"tra_ope":"eah2012","tra_for":"AJH1","tra_mat":""}
        */
    }
    ...
    function para_proceso_planilla_carga_o_recepcion($rol,$que_planilla,$tipo='normal'){
        if($rol){
            $tabla_personal=$this->nuevo_objeto("Tabla_opciones");
            $tabla_personal->leer_unico(array(
                    'opc_ope'=>$GLOBALS['NOMBRE_APP'],
                    'opc_conopc'=>'roles_personas',                
                    'opc_opc'=>$rol
                ));
            $rol_descripcion=$tabla_personal->datos->opc_texto;
        }else{
            $rol_descripcion="";
        }
        return new Proceso_generico(array(
            'titulo'=>($tipo=='correccion'?'Correcciones especiales de la TEM':($tipo=='mis.sup.tel'?'Mis supervisiones telefónicas':($rol?"Planilla de $que_planilla del $rol_descripcion":"Planilla monitoreo de la TEM"))),
            ...
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) use ($rol,$rol_descripcion,$que_planilla,$tipo){
                if($que_planilla=='carga'){
                    mostrar_planilla_carga($este,$rol, $rol_descripcion);
                }else{
                    mostrar_planilla_recepcion($este,$rol, $rol_descripcion,$tipo);
                }
            }
        ));
    }
    function proceso_planilla_carga_encuestador(){
        return $this->para_proceso_planilla_carga_o_recepcion(1,'carga');
    }
    ...
    function proceso_planilla_recepcion_encuestador(){
        return $this->para_proceso_planilla_carga_o_recepcion(1,'recepción');
    }
    function proceso_planilla_recepcion_recuperador(){
        return $this->para_proceso_planilla_carga_o_recepcion(2,'recepción');
    }
    function proceso_planilla_recepcion_supervisor_campo(){
        return $this->para_proceso_planilla_carga_o_recepcion(3,'recepción');
    }
    function proceso_planilla_recepcion_supervisor_telefonico(){
        return $this->para_proceso_planilla_carga_o_recepcion(4,'recepción');
    }
    function proceso_planilla_mis_superviones_telefonicas(){
        return $this->para_proceso_planilla_carga_o_recepcion(4,'recepción','mis.sup.tel');
    }
    function proceso_planilla_monitoreo_TEM(){
        return $this->para_proceso_planilla_carga_o_recepcion(null,'recepción');
    }
    function proceso_correccion_TEM(){
        return $this->para_proceso_planilla_carga_o_recepcion(null,'recepción','correccion');
    }
    function proceso_confirmar_salida_campo_papel(){
        return new Proceso_confirmar_salida_campo_papel();
    }
    function proceso_averiguar_numero_carga(){
        return new Proceso_averiguar_numero_carga();
    }
                ...
                if($_SESSION["{$GLOBALS['NOMBRE_APP']}_usu_blanquear_clave"]){
                    ...
                    $este->salida->abrir_grupo_interno();
                        if(isset($_SESSION['ir_despues_de_loguearse'])){
                            $este->salida->enviar_link((termina_con($_SESSION['ir_despues_de_loguearse'],'cargar_dispositivo')?'Ir a CARGAR DISPOSITIVO':'Ir al sitio donde estaba antes'),'',$_SESSION['ir_despues_de_loguearse']);
                        }
                    $este->salida->cerrar_grupo_interno();
                }
            }
        ...
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este){
                $este->salida->abrir_grupo_interno('cabezal_matriz');
                    ...
                    $este->salida->enviar(AUTF8_NBSP.AUTF8_NBSP.AUTF8_NBSP.AUTF8_NBSP.AUTF8_NBSP.'HOJA DE RUTA V 1.36 '.AUTF8_NBSP.AUTF8_NBSP.AUTF8_NBSP,'',array('tipo'=>'span'));
...
JS
    ...
    function proceso_grilla_tabla_inconsistencias(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,            
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'inconsistencias',array('inc_ope'=>$GLOBALS['NOMBRE_APP']));
            }
        ));
    }
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_eah2012());
}
?>