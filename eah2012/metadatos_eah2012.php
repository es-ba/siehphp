...
class Metadatos_eah2012 extends Objeto_de_la_base{ 
    ...
    function ejecutar_instalacion(){
        ...
        if($via_json){
            $separador=";/*FIN*/\n";
            $json=json_decode(json_arreglar(file_get_contents('..\operaciones_eah2012\eah2012_dump.json')),true);
            if(!$json){
                throw new Exception_Tedede("Error al parsear el json eah2012_dump.json. Error ".json_str_error());
            }
            $sentencias="";
            foreach($json['tablas'] as $tabla=>$contenido){
                ...
                $sentencias.=json_generar_insert($GLOBALS['esquema_principal'],$tabla,$contenido,$separador,$json['jerarquia']);
            }
        }else{
            $separador=";\n";
            $sentencias=file_get_contents('..\operaciones_eah2012\migraciones\metadatos.sql');
        }
    ...
    function instalar_tem_de_prueba(){
        ...
        if($cuantos=$tabla_tem->contar_cuantos(array())){ // CORRECTO UN = DE ASIGNACIÓN
            ...
            $this->contexto->db->ejecutar_sql(new Sql("INSERT INTO claves (cla_ope, cla_for, cla_mat, cla_enc, cla_tlg)   SELECT 'eah2012','TEM','',tem_enc,tem_tlg     FROM tem; "));
        }
    }
