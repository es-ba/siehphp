...
function json_dump($esquema,$tabla,$jerarquia=false,$contexto=array(),$profundidad=0,$campos_excluir=array()){
	$str=''; 
	if(is_array($tabla)){
		$str.="{ json_dumper:1\n";
        if($jerarquia){
            $str.=", jerarquia:".json_encode($jerarquia)."\n";
        }
		$str.=", tablas:\n";
		$str.="  { ";
		$separador="";
		foreach($tabla as $una_tabla){
			$str.=$separador.$una_tabla.":\n".json_dump($esquema,$una_tabla,$jerarquia,array(),$profundidad+1,$campos_excluir);
			$separador="\n  , ";
		}
		$str.="\n  }\n}\n";
	}else{
		...
		while($fila=$cursor->fetch(PDO::FETCH_ASSOC)){
			...
			foreach($fila as $nombre_columna=>$valor){
				if(!isset($contexto[$nombre_columna])){
                    ...
                    if(!isset($campos_excluir[$nombre_columna])){
            ...
            $def_hijas=array();
            if($jerarquia){
                foreach($jerarquia as $tabla_hija=>$tabla_padre){
                    if($tabla==$tabla_padre){
                        $def_hijas[$tabla_hija]=$db->nombre_fk($esquema,$tabla_padre,$esquema,$tabla_hija);
                    }
                }
            }else{
                $fk_hijas=$db->dependientes_fk($esquema,$tabla);
                foreach($fk_hijas as $fk_hija){
                    $def_hijas[$fk_hija->tabla]=$fk_hija->nombre_fk;
                }
            }
			foreach($def_hijas as $tabla_hija=>$fk_hija_nombre){
				$campos_join=$db->campos_union_fk($esquema,$fk_hija_nombre);
				$nuevo_contexto=array();
				$pk_de_la_hija=$db->campos_pk($esquema,$tabla_hija);
				...
				if($todos_campos_hijo_son_pk || $jerarquia){
					$dump_hija=json_dump($esquema,$tabla_hija,$jerarquia,$nuevo_contexto,$profundidad+1,$campos_excluir);
...
function json_generar_insert_aux(&$respuestas, $esquema, $tabla, $datos, $jerarquia=false, $forzar_campos=array(), $campos_pk_madre=array()){
	global $db;
    ...
    if(!is_array_o_stdclass($datos)){
        throw new Exception_Tedede("json_generar_insert_aux: $$datos={{$datos}} en $tabla para ".json_encode($campos_pk_madre));
    }
	foreach($datos as $linea){
		...
		foreach($db->dame_arreglo_campos($esquema,$tabla) as $nombre_columna){
			...
			}else if(@$forzar_campos[$nombre_campo]){
                $valor=$forzar_campos[$nombre_campo];
            }else{
        ...
        foreach($def_hijas as $tabla_hija=>$fk_hija_nombre){
			...
			if(array_key_exists($tabla_hija,$linea)){
				...
				if($todos_campos_hijo_son_pk || $jerarquia){
					json_generar_insert_aux($respuestas, $esquema, $tabla_hija, $linea[$tabla_hija], $jerarquia, $forzar_campos, $campos_pk);
				}
			}
		}
	}
    // Loguear('2013-06-14', "[$tabla]=$datos_insert", null, null, true);
	$respuestas[$tabla].=$datos_insert;
}

function json_generar_insert($esquema, $tabla, $json, $separador=";\n", $jerarquia=false, $forzar_campos=array()){
	$respuestas=array();
    if(is_string($json)){
	...
	json_generar_insert_aux($respuestas, $esquema, $tabla, $datos, $jerarquia, $forzar_campos);
	$str=implode($separador, $respuestas);
	if($str){
