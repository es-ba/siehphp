...
$GLOBALS['titulo_corto_app']="etoi221";

$es_teletoi_desa = false;

$GLOBALS['es_tele_etoi']=strpos(__DIR__,"alserver_tele_etoi221/")>0 || !strpos(__DIR__,"alserver_etoi221/")>0 && $es_teletoi_desa;
...
require_once "metadatos_etoi221.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../etoi221");

class Aplicacion_etoi221 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            ...
            $this->salida->html_title="CAPA - etoi221";
        }else{
            $this->salida->html_title="TEST - etoi221";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                //$this->salida->manifiesto="etoi221.manifest";
        ...
        $this->salida->agregar_js('dbo_etoi221.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_etoi221(), /// aca inserta los metadatos corriendo '..\operaciones_etoi221\etoi221_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_etoi221\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_etoi221\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_etoi221();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'etoi221',"),array(PRIMER_TLG,"'etoi221',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_etoi221::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_etoi221\actualizar_a_etoi221.sql'))));                    
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_etoi221");
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_etoi221());
}
?>
