...
set search_path=encu, public, comun;
  
INSERT INTO encu.varcal(
            varcal_ope, varcal_varcal, varcal_destino, varcal_orden, varcal_nombre, 
            varcal_comentarios, varcal_tlg, varcal_activa, varcal_tipo,varcal_baseusuario, varcal_nombrevar_baseusuario, varcal_tipodedato, varcal_valida)
    VALUES (dbo.ope_actual(), 'mie_bu', 'mie',4800 , 'Número de miembro para Base usuarios',null, 1, TRUE, 'especial', TRUE, 'mie_bu','entero', true);
...
select i.pla_enc, i.pla_hog, i.pla_mie, pla_mie_bu , pla_p4 from encu.plana_i1_ i 
inner join plana_s1_p p on i.pla_enc = p.pla_enc and i.pla_hog = p.pla_hog and i.pla_mie = p.pla_mie 
--where pla_p4=1 and i.pla_mie_bu  is distinct from 1
...
update encu.respuestas set res_valor = '0'
 where res_ope =dbo.ope_actual() and res_for='TEM' and res_mat = '' and res_enc = 130009 and res_hog = 0 and res_mie = 0 and res_var = 'fin_anacon';
...
update encu.respuestas set res_valor = 2
 where res_ope =dbo.ope_actual() and res_for='S1' and res_mat = 'P' and res_enc = 130009 and res_hog = 1 and res_mie = 1 and res_var = 'p4';
update encu.respuestas set res_valor = 1P
 where res_ope =dbo.ope_actual() and res_for='S1' and res_mat = 'P' and res_enc = 130009 and res_hog = 1 and res_mie = 4 and res_var = 'p4';
...
update encu.respuestas set res_mie = 4 where res_ope = dbo.ope_actual() and res_for='S1' and res_mat = 'P' and res_enc = 130016 and res_hog = 1 and res_mie = 3;
update encu.respuestas set res_mie = 4 where res_ope = dbo.ope_actual() and res_for='I1' and res_mat = '' and res_enc = 130016 and res_hog = 1 and res_mie = 3;
