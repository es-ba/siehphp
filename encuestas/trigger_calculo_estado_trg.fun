  ...
  then
    if new.res_var='verificado_enc' and new.res_valor::integer<>2 then
        ...
        if (v_rea_enc = 1 or dbo.norea_supervisable(v_norea_enc)) and v_dominio=3 then
            if v_calculo_enc = 4 and coalesce(v_hog_tot,0)<=1 then
                v_aux_aleat_enc := '3';
            elsif v_calculo_enc = 3 and v_rea_enc = 1 and coalesce(v_hog_tot,0)<=1 then
                v_aux_aleat_enc='4';
            else
        ...
        if (v_rea_recu = 3 or dbo.norea_supervisable(v_norea_recu)) and v_dominio = 3 then
            if v_calculo_recu =7 and 'si es la misma persona no hay supervision de campo'='x' then
                v_aux_aleat_recu := '3';
            elsif v_calculo_recu = 6 and v_rea_recu = 3 then
                v_aux_aleat_recu := '4';
            else
