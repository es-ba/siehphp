...
CREATE OR REPLACE FUNCTION dbo.mediana_expandida_agrupada(p_variable text, p_filtro text, p_groupby text, p_valor integer, p_modo text)
  RETURNS decimal AS 
$BODY$ 
DECLARE
  v_resultado decimal;
...
ALTER FUNCTION dbo.mediana_expandida_agrupada(text, text, text, integer, text)
  OWNER TO tedede_php;
  
##CASOS_SQL
-- set search_path = encu, comun, public;
-- select dbo.mediana_expandida_agrupada('e_aesc', '(pla_edad >= 25 and pla_e_aesc is distinct from 99 AND pla_e_aesc is distinct from 98) AND pla_estado>=77 AND pla_rea not in (0,2) AND pla_entrea <> 4','comuna',1,'');
