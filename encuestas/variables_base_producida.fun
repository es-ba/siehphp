...
NUEVA
##PROVISORIO
-- Function: encu.variables_base_producida(text)

*/
-- 
DROP FUNCTION IF EXISTS encu.variables_base_producida(text);
DROP FUNCTION IF EXISTS encu.variables_base_producida(text,text);
DROP FUNCTION IF EXISTS encu.variables_base_producida(text,text, integer, integer);
DROP FUNCTION IF EXISTS encu.variables_base_producida(text,text, integer, integer, text);
    ...
    FOR v_var_seleccionadas in
       select pre_blo, table_name, column_name as var_bu, 
              ...
              case when table_name = 'pla_ext_hog' and varcal_destino in ('hog','tem') then 'hogar'
                   ...
                   when table_name = 'pla_ext_hog' and column_name='pla_modo' then 'hogar'
...
--select encu.variables_base_producida('basehogar','cedem');
--select encu.variables_base_producida('basepersonas','cedem');

--select encu.variables_base_producida('basehogar','base_cv');
--select encu.variables_base_producida('basepersonas','base_cv');

--select encu.variables_base_producida('basehogar','impu_ing');
--select encu.variables_base_producida('basepersonas','impu_ing');
-- select encu.variables_base_producida('baseexm','prueba_exmiem');
-- select encu.variables_base_producida('baseexm','cp14_exm_entrega');
select encu.variables_base_producida('baseexm','cp14_exm_entrega');
select encu.variables_base_producida('basehogar','cp14_viv_entrega');

-- select * from encu.ebcp2014_cp14_exm_entrega_exmiem;
--select * from encu.ebcp2014_cp14_viv_entrega_hogar;

-- select encu.variables_base_producida('basehogar','completa',77,77,'ETOI');

--eah2017
--select encu.variables_base_producida('basehogar','para_fexp','77','79','Completa',0,0);
--empa171
--select encu.variables_base_producida('baseper','rea_per','77','79','',0,2)