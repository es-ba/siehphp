##FUN
madre_con_hijos_menor_de
##ESQ
dbo
##PARA
revisar 
##DETALLE
en eah2013 y ebcp2014 existía y es distinta porque sigue estando la variable f_nac_o
NUEVA
##PROVISORIO
CREATE OR REPLACE FUNCTION dbo.madre_con_hijos_menor_de(p_enc integer, p_hog integer, p_mie integer, p_edad integer, p_unidad_tiempo integer)
  RETURNS boolean AS
$BODY$
DECLARE
 v_respuesta boolean;
  ...
  case p_unidad_tiempo
     ...
     when 2 then
	  select case when count(p.pla_mie)>0 then true else false end into v_respuesta from encu.plana_s1_p p
                                                                                           ...
                                                                                           and dbo.meses_a_la_fecha(
                                                                                           fechadma(pla_f_nac_d, pla_f_nac_m,pla_f_nac_a)
                                                                                           , pla_f_realiz_o) < p_edad;
     else
	  v_respuesta:=false;
  end case;
  return v_respuesta;
...
/*OTRA*/
ALTER FUNCTION dbo.madre_con_hijos_menor_de(integer, integer, integer, integer, integer)
  OWNER TO tedede_php;