...
BEGIN  
    ...
    INSERT INTO encu.consistencias( con_ope,con_con,con_precondicion,con_rel,
        ...
        SELECT  var_ope, 'audi_nsnc_'||var_var, 'informado('||var_var||')' as precondicion, xcon_rel,
                ...
                xcon_activa, 'Variable ' ||var_var ||' Preg:'||var_pre|| ' For:'||var_for||
                    ' tiene NS/NC(-9) o no posee dato (-1). Para los casos sin dato intente recuperar el mismo.'||
                    ' Cuando no se pueda obtener la información faltante reemplace el guión guión (--) por barra barra (//)' as con_explicacion, 
                    xcon_tipo, xcon_falsos_positivos,
                xcon_importancia, xcon_momento, xcon_grupo, xcon_gravedad, 1
            FROM encu.variables_ordenadas 
                left join encu.opciones on var_ope= opc_ope and var_conopc=opc_conopc and opc_texto ilike 'No sabe'
            WHERE var_ope=poperativo and var_for is distinct from 'SUP'
                and opc_texto is null
            ORDER BY orden; 
END;
$BODY$
LANGUAGE plpgsql ;
