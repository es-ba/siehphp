...
as $$ select regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace(regexp_replace($1,
                 '(?!\mAND\M)(?!\mOR\M)(?!\mNOT\M)(?!\mIS\M)(?!\mNULL\M)(?!\mIN\M)(?!\mCASE\M)(?!\mWHEN\M)(?!\mTHEN\M)(?!\mELSE\M)(?!\mEND\M)(?!\mTRUE\M)(?!\mFALSE\M)(?!\mEXISTS\M)(?!\mDISTINCT\M)(?!\mFROM\M)(?!\mBETWEEN\M)(?!\mAS\M)((\m[a-z][a-z0-9_.]*?\M)|\$\$[^$]*?\$\$|"@[^"]*?@"\()(?!\.)(?!\s*\()',
                 '¬¬¬¬1234¬¬¬¬\1¬¬¬¬4321¬¬¬¬','ig'),
                 '¬¬¬¬1234¬¬¬¬(\$\$.*?\$\$)¬¬¬¬4321¬¬¬¬','\1::text','ig'),
                 '::¬¬¬¬1234¬¬¬¬(.*?)¬¬¬¬4321¬¬¬¬','::\1','ig'),
                 '¬¬¬¬1234¬¬¬¬(\W.*?)¬¬¬¬4321¬¬¬¬','\1','ig'),
...
select expresion, reemplazante, esperado, comun.reemplazar_variables(expresion, reemplazante) as obtenido
  from (select 'uno+dos' as expresion,'v_\1' as reemplazante,'v_uno+v_dos' as esperado
        ...
        union select 'ing_tot=2 and ping=3', 'v_\1', 'v_ing_tot=2 and v_ping=3'
        ...
        union select 'case when algo then 1::text else 2::bigint end', 'v_\1', 'case when v_algo then 1::text else 2::bigint end' 
