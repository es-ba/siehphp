...
class Proceso_borrar_md extends Proceso_Formulario{
    function __construct(){
        parent::__construct(array(
            'titulo'=>'Eliminar modulo discapacidad',
    ...
    function responder(){
        ...
        if(!$this->argumentos->tra_confirmar1
            ...
            $cursor=$this->db->ejecutar_sql(new Sql(<<<SQL
                  select pla_edad as edad
                  from plana_s1_p
                  where pla_enc=:tra_enc and pla_hog=:tra_hog and pla_mie=:tra_mie 
SQL
                ,array(
            ...
            if(!$datos){
                ...
                if($datos->edad!==$this->argumentos->tra_edad){
                    ...
                    , <<<SQL
                        delete from claves
                           where cla_ope='{$GLOBALS['NOMBRE_APP']}' 
                             ...
                             and cla_exm=0
SQL
                    , "delete from plana_md_ where pla_enc=:tra_enc and pla_hog=:tra_hog and pla_mie=:tra_mie and pla_exm=0"
                    ) as $sql_parcial)
                {
                    if($sql_parcial){
