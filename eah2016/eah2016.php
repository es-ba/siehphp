...
class Aplicacion_eah2016 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EAH2016";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - eah2016";
        }else{
            $this->salida->html_title="TEST - eah2016";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="eah2016.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_eah2016.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_eah2016(), /// aca inserta los metadatos corriendo '..\operaciones_eah2016\eah2016_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_eah2016\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_eah2016\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_eah2016();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'eah2016',"),array(PRIMER_TLG,"'eah2016',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_eah2016::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_eah2016\actualizar_a_eah2016.sql'))));                    
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_eah2016");
    ...
    */    
    ...
    function proceso_grilla_S1_variables_externas(){
        return new Proceso_generico(array(
            'titulo'=>(substr($GLOBALS['NOMBRE_APP'],0,3)=='eah'&& $GLOBALS['anio_operativo']>=2015)?'Grilla de Variables calculadas externas del S1 - Modo '. $_SESSION['modo_encuesta']:'Grilla de Variables calculadas externas del S1',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_eah2016());
}
?>