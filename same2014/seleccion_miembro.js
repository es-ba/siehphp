...
window.addEventListener('load',function(){
    if(document.getElementById('var_tp') && operativo_actual=='same2014'){
...
function seleccionar_miembro(solo_controla,este){
    ...
    for(var num_miembro=1; num_miembro<=100; num_miembro++){    
        ...
        if (ud_este){
            ...
            if ((letra_manual && uds_miembros[num_miembro].var_l0) || (!letra_manual && ud_este.var_edad>=16 && ud_este.var_edad<=65)){
    ...
    if(solo_controla){
        ...
        if(grabar_si_es_necesario_o_seguir()){
            setTimeout(function(){
                ir_a_url('same2014.php?hacer=desplegar_formulario&todo={"tra_ope":"same2014","tra_for":"S1","tra_mat":""}');
            },200);
        }
    }
}
