...
class Proceso_eliminar_hogar__same2014 extends Proceso_Formulario{
    function __construct(){
        parent::__construct(array(
            'titulo'=>'Eliminar Hogar (SAME)',
...
SQL
                ...
                $cursor=$this->db->ejecutar_sql(new Sql(<<<SQL
                    select count(nullif(trim(pla_nombre),'')) as cantidad,
                           ...
                           string_agg(case when pla_mie=:tra_mie then coalesce(trim(pla_nombre),'')||','||coalesce(pla_edad,0) else '' end,'') as nombre_edad
                      from plana_s1_p
                    ...
                    , <<<SQL
                        delete from claves
                           where cla_ope='{$GLOBALS['NOMBRE_APP']}' 
                             ...
                             and cla_hog=:tra_hog
SQL
                    , "delete from plana_s1_  where pla_enc=:tra_enc and pla_hog=:tra_hog"
                    , "delete from plana_s1_p where pla_enc=:tra_enc and pla_hog=:tra_hog"
                    , "delete from plana_i1_  where pla_enc=:tra_enc and pla_hog=:tra_hog"
                    ) as $sql_parcial)
                {
