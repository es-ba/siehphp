  ...
  LOOP
    ...
    LOOP
        ...
        LOOP
           ...
           v_expresion:= regexp_replace(v_expresion, '\mnew.pla_(edad|sexo|f_nac_o|p5|comuna|estado|h2|h3|v2)\M'::text, 'v_\1'::text,'ig');
           ...
           v_expresion_valor:= regexp_replace(v_expresion_valor, '\mnew.pla_(edad|sexo|f_nac_o|p5|comuna|h2|h3|v2)\M'::text, 'v_\1'::text,'ig');
...
-- en el caso de variables del bloque Viv, hay que leer el hogar 1
        ...
        EXCEPTION
          WHEN OTHERS THEN
            ...
            return 'ERROR: Compilación de variable con error. Variable: '|| v_variable_mensaje||'. Error '|| sqlstate || ': ' || sqlerrm;
        END; 
        if(p_cual<>'#todo')then 
...
$BODY$
    BEGIN
    if new.pla_edad is distinct from old.pla_edad or new.pla_sexo is distinct from old.pla_sexo or new.pla_f_nac_o is distinct from old.pla_f_nac_o or new.pla_p5 is distinct from old.pla_p5 then
