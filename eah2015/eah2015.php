...
$GLOBALS['titulo_corto_app']="EAH 2015";

require_once "lo_imprescindible.php";
require_once "aplicaciones.php";
require_once "tabla_operativos.php";
require_once "metadatos_eah2015.php";
require_once "todos_los_php.php";
//require_once "../encuestas/proceso_setear_modo_encuesta.php";

incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../eah2015");

class Aplicacion_eah2015 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EAH 2015";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - eah2015";
        }else{
            $this->salida->html_title="TEST - eah2015";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="eah2015.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_eah2015.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_eah2015(), /// aca inserta los metadatos corriendo '..\operaciones_eah2015\eah2015_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_eah2015\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_eah2015\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_eah2015();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'eah2015',"),array(PRIMER_TLG,"'eah2015',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_eah2015::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_eah2015\actualizar_a_eah2015.sql'))));                    
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_eah2015");
    ...
    function proceso_grilla_personal(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'submenu'=>'campo',
            'permisos'=>array('grupo'=>'subcoor_campo','grupo1'=>'ana_campo'),
            'funcion'=>function(Procesos $este){
    ...
    */    
        ...
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_salud',array(
                    ...
                    pla_sn1_3=1|pla_sn1_3_esp!=NULL|pla_sn1_4=1|pla_sn1_4_esp!=NULL|pla_sn1_5=1|pla_sn14=4|pla_sn14_esp!=NULL'
                )                    
                ,null,array());
            }            
        ));
    ...
    function proceso_grilla_registro_claves(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'submenu'=>'recepción',
            'permisos'=>array('grupo1'=>'recepcionista','grupo2'=>'mues_campo'),
            'funcion'=>function(Procesos $este){
   ...
   function proceso_grilla_sup_educacion(){
    ...
    /*
    function proceso_grilla_monitoreo_viv_actividad(){
        return new Proceso_generico(array(
            'titulo'=>'Monitoreo de control de cambios por condicion de actividad',
            'permisos'=>array('grupo'=>'programador', 'grupo1'=>'procesamiento'),
            'para_produccion'=>true,
            'submenu'=>'procesamiento',
            'funcion'=>function(Procesos $este){
                $este->salida->enviar('Datos del operativo anterior, actualizados cada 24 hs');
                enviar_grilla($este->salida,'monitoreo_viv_actividad' );
            }
        ));
    }
    */    
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_eah2015());
}
?>