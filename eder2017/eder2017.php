...
$GLOBALS['titulo_corto_app']="EDER 2017";

$es_teletoi_desa = false;

$GLOBALS['es_tele_etoi']=strpos(__DIR__,"alserver_tele_etoi173/")>0 || !strpos(__DIR__,"alserver_etoi173/")>0 && $es_teletoi_desa;
...
require_once "metadatos_eder2017.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../eder2017");

class Aplicacion_eder2017 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EDER 2017 PRUEBA PILOTO";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - eder2017";
        }else{
            $this->salida->html_title="TEST - eder2017";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="eder2017.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_eder2017.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_eder2017(), /// aca inserta los metadatos corriendo '..\operaciones_eder2017\eder2017_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_eder2017\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_eder2017\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_eder2017();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'eder2017',"),array(PRIMER_TLG,"'eder2017',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_eder2017::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_eder2017\actualizar_a_eder2017.sql'))));                    
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_eder2017");
    ...
    */    
    ...
    function proceso_grilla_I1_rama_p(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Preguntas para codificar rama ant. padre',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_rama_ocupacion_p',
                array(
                    'tem_estado'=>'#>=25',
                ),
                null,array());
    ...
    function proceso_grilla_I1_rama_m(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Preguntas para codificar rama ant. madre',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_rama_ocupacion_m',
                array(
                    'tem_estado'=>'#>=25',
                ),
                null,array());
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_eder2017());
}
?>