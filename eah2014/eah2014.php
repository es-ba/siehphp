...
$GLOBALS['titulo_corto_app']="E.A.H. 2014";

require_once "lo_imprescindible.php";
require_once "aplicaciones.php";
require_once "tabla_operativos.php";
require_once "metadatos_eah2014.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../eah2014");

class Aplicacion_eah2014 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EAH 2014";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - eah2014";
        }else{
            $this->salida->html_title="TEST - eah2014";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="eah2014.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_eah2014.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        new Metadatos_eah2014(), /// aca inserta los metadatos corriendo '..\operaciones_eah2014\eah2014_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                  //'..\operaciones_eah2014\insercion_tabla_tem.sql',
...
// OJO 2014 FALTA                 //'..\operaciones\insercion_tabla_est_var.sql', 
                                  '..\operaciones\vista_var_orden.sql',// ok
                                   ) as $archivo)
                    {
                        $sentencias_generales=explode("/*OTRA*/",file_get_contents($archivo));
                        $sql_sentencias=str_replace(array('/*CAMPOS_AUDITORIA*/',"'etoi143',"),array(PRIMER_TLG,"'dbo.ope_actual()',"),$sentencias_generales);
                        if(!$esta_es_la_base_en_produccion){
                            $este->db->beginTransaction();
                    ...
                    $estructura_encuesta = new Estructura_eah2014();
                    ...
                    $este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'eah2014',"),array(PRIMER_TLG,"'eah2014',"),file_get_contents($archivo))));
                    if($esta_es_la_base_en_produccion){
                        $este->db->commit();
                    }
                }catch(Exception $e){
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_eah2014::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_eah2014\actualizar_a_eah2014.sql'))));                    
            // esta sentencia se incluye a partir de eah2014 en '..\operaciones_eah2014\otras_sentencias_instalacion.sql'                    
            $estructura_encuesta = new Estructura_eah2014();
            $estructura_encuesta->contexto=$este;
            $estructura_encuesta->generar_estructura();
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_eah2014");
    ...
    function proceso_grilla_A1(){
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este) {           
               ...
               array('pla_v2'=>'# =8|pla_h2=7|pla_v5=4|pla_v5_esp!=NULL|pla_h20_14=1|pla_h20_esp!=NULL'),
               null,array());                
            }
        ));
    }
    function proceso_grilla_A1_X_abiertas(){
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                array('a1_x5'=>'# =1|a1_x5_tot >0|pla_sexo_ex>0|pla_pais_nac>0|pla_edad_ex >0|pla_niv_educ >0|
                       pla_anio >0|pla_lugar >0|pla_lugar_esp1!=NULL|pla_lugar_esp2!=NULL|pla_lugar_esp3!=NULL'
                ),
                null,array());                
        ...
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                array('pla_t8'=>'# =3|
                       ...
                       pla_t48b=6|pla_t48b_esp!=NULL|
                       pla_t18=1|pla_t23!=NULL|pla_t39_bis2=8|pla_t39_bis2_esp!=NULL|                       
                       pla_t24!=NULL|pla_t25!=NULL|pla_t26!=NULL|
                       pla_i3_10=1|                       
                       pla_i3_10x>=0|
                       pla_i3_10_otro!=NULL'                       
                ),                
        ...
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_salud',array(
                    'pla_sn1_1'=>'#=1|pla_sn1_7=1|pla_sn1_2=1|pla_sn1_3=1|pla_sn1_4=1|pla_sn5=1|
                    pla_sn4=4|pla_sn4_esp!=NULL|PLA_sn5=9|pla_sn5_esp!=NULL|pla_sn14=4|pla_sn14_esp!=NULL|pla_sn15k=1|pla_sn15k_esp!=NULL'
                )                    
                ,null,array());
            }            
        ));
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_eah2014());
}
?>