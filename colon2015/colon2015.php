...
$GLOBALS['titulo_corto_app']="COLON 2015";

require_once "lo_imprescindible.php";
require_once "aplicaciones.php";
require_once "tabla_operativos.php";
require_once "metadatos_colon2015.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../colon2015");

//$_SESSION['modo_encuesta']=''; 
class Aplicacion_colon2015 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="COLON 2015";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - colon2015";
        }else{
            $this->salida->html_title="TEST - colon2015";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="colon2015.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_colon2015.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_colon2015(), /// aca inserta los metadatos corriendo '..\operaciones_colon2015\colon2015_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_colon2015\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_colon2015\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_colon2015();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'colon2015',"),array(PRIMER_TLG,"'colon2015',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_colon2015::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_colon2015\actualizar_a_colon2015.sql'))));                    
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_colon2015");
    ...
    */
    ...
    function proceso_alta_tem(){
        return new Proceso_alta_tem();
    }
    ...
    function proceso_grilla_consistencias(){
        return new Proceso_generico(array(
            ...
            'permisos'=>array('grupo'=>'procesamiento','grupo1'=>'mues_campo','grupo2'=>'ana_sectorial'),
            'submenu'=>'consistencias',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_colon2015());
}
?>