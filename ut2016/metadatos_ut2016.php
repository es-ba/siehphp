...
class Metadatos_ut2016 extends Objeto_de_la_base{ 
    ...
    function ejecutar_instalacion($con_dependientes=true){
        ...
        if($via_json){
            $separador=";/*FIN*/\n";
            $json=json_decode(json_arreglar(file_get_contents('..\operaciones_ut2016\ut2016_dump.json')),true);
            if(!$json){
                throw new Exception_Tedede("Error al parsear el json ut2016_dump.json. Error ".json_str_error());
            }
            $sentencias="";
            ...
            $separador=";\n";
            $sentencias=file_get_contents('..\operaciones_ut2016\migraciones\metadatos.sql');
        }
    ...
    function instalar_tem_de_prueba(){
        ...
        if($cuantos=$tabla_tem->contar_cuantos(array())){ // CORRECTO UN = DE ASIGNACIÓN
            ...
            $this->contexto->db->ejecutar_sql(new Sql("INSERT INTO claves (cla_ope, cla_for, cla_mat, cla_enc, cla_tlg)   SELECT 'ut2016','TEM','',tem_enc,tem_tlg     FROM tem; "));
        }
    }
...
class Tablas_planas extends Objeto_de_la_base{ 
    ...
    function ejecutar_instalacion($con_dependientes=true){
        ...
        while($tabla_matrices->obtener_leido()){
            if ($tabla_matrices->datos->mat_for=='SUP'/*$tabla_matrices->datos->mat_for!='TEM' */){  //esta condición es para no volver a generar la plana_tem_ cuando la misma ya está generada y con información cargada
