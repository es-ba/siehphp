...
class Aplicacion_ut2016 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="UT 2016";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - ut2016";
        }else{
            $this->salida->html_title="TEST - ut2016";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="ut2016.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_ut2016.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_ut2016(), /// aca inserta los metadatos corriendo '..\operaciones_ut2016\ut2016_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_ut2016\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_ut2016\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_ut2016();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'ut2016',"),array(PRIMER_TLG,"'ut2016',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_ut2016::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
                {
 //                   if($objeto_de_la_base!=='tabla_plana_TEM_'){
                        $este->salida->enviar('instalando '.get_class($objeto_de_la_base));
                        $objeto_de_la_base->contexto=$este;
                        $objeto_de_la_base->ejecutar_instalacion();
//                    }
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_ut2016\actualizar_a_ut2016.sql'))));                    
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_ut2016");
...
*/    
    function proceso_grilla_especificaciones_viv_hog(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Especificaciones de Vivienda y Hogar',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'ut_especificaciones_viv_hog',array(
                    'pla_h2'=>'#=7|pla_h2_esp!=NULL|pla_v2=8|pla_v2_esp!=NULL|pla_h20a_14=1|pla_h20a_14_esp!=NULL'
                )                    
                ,null,array());
            }            
        ));
    }
    function proceso_grilla_ayudas_recibidas_hogar(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Ayudas recibidas por el hogar',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'ut_ayudas_recibidas_hogar',array(
                    'pla_uh1'=>'#=1|pla_uh3=5|pla_uh3_esp!=NULL|pla_uh4=4|pla_uh4_esp!=NULL|pla_uh7=5|pla_uh7_esp!=NULL|pla_uh8=4|pla_uh8_esp!=NULL'||'pla_uh14=4|pla_uh14_esp!=NULL|pla_uh16=4|pla_uh16_esp!=NULL|pla_uh21=5|pla_uh21_esp!=NULL|pla_uh22= 4|pla_uh22_esp!=NULL'
                )                    
                ,null,array());
            }            
        ));
    }
    function proceso_grilla_especificaciones_migraciones(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Especificaciones de migraciones',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'ut_especificaciones_migraciones',array(
                    'pla_m1'=>'#=(2|3|4)|pla_m1_esp2!=NULL|pla_m1_esp3!=NULL|pla_m1_esp4!=NULL'
                )                    
                ,null,array());
            }            
        ));
    ...
    function proceso_grilla_registro_claves(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'submenu'=>'recepción',
            'permisos'=>array('grupo1'=>'recepcionista','grupo2'=>'mues_campo','grupo3'=>'procesamiento'),
            'funcion'=>function(Procesos $este){
    ...
    function proceso_grilla_baspro_var_ut(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de variables de Bases Producidas UT',
            'permisos'=>array('grupo'=>'programador', 'grupo1'=>'procesamiento'),
            'para_produccion'=>true,
            'submenu'=>'procesamiento',
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'baspro_var_ut',array(),false,array('otras_opciones'=>array('agregando_filas_completas'=>true)));
            }
        ));
    }
    function proceso_exportar_base_ut(){
        return new Proceso_exportar_base_ut();
    }
}
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_ut2016());
}
?>