...
dbo.cant_rango_edad_excluyendo_mie=function(encues,nhogar,nedad1, nedad2, nmiembro){
    var cant=0;
    var nro_mie=1;
    var ud_este;
    var pk_este=cambiandole(pk_ud,{tra_for:'S1', tra_mat:'P', tra_enc:encues, tra_hog:nhogar, tra_mie:nro_mie}); 
    ud_este=otras_rta[JSON.stringify(pk_este)];
    while(ud_este || ud_este!=undefined){
        if(ud_este.var_edad>=nedad1 && ud_este.var_edad<=nedad2 && pk_este.tra_mie!=nmiembro){
            cant++;    
        }
        nro_mie++;
        pk_este=cambiandole(pk_este,{tra_mie:nro_mie}); 
...
dbo.cant_mayores_de_excluyendo_mie=function(encues,nhogar,nedad, nmiembro){
    var cant=0;
    var nro_mie=1;
    var ud_este;
    var pk_este=cambiandole(pk_ud,{tra_for:'S1', tra_mat:'P', tra_enc:encues, tra_hog:nhogar, tra_mie:nro_mie}); 
    ud_este=otras_rta[JSON.stringify(pk_este)];
    while(ud_este || ud_este!=undefined){
        if(ud_este.var_edad>nedad && pk_este.tra_mie!=nmiembro){
            cant++;    
        }
        nro_mie++;
        pk_este=cambiandole(pk_este,{tra_mie:nro_mie}); 
...
dbo.cant_p5_x_hog=function(encues,nhogar,valp5){
    var cant=0;
    var nro_mie=1;
    var ud_este;
    var pk_este=cambiandole(pk_ud,{tra_for:'S1', tra_mat:'P', tra_enc:encues, tra_hog:nhogar, tra_mie:nro_mie}); 
