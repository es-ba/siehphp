...
$GLOBALS['anio_operativo']=2023;
//$GLOBALS['trimestre_operativo']=1;
$GLOBALS['esquema_principal']='encu';
$GLOBALS['titulo_corto_app']="vcm2023";

$es_teletoi_desa = false;

$GLOBALS['es_tele_etoi']=strpos(__DIR__,"alserver_tele_vcm2023/")>0 || !strpos(__DIR__,"alserver_vcm2023/")>0 && $es_teletoi_desa;
...
require_once "metadatos_vcm2023.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../vcm2023");

class Aplicacion_vcm2023 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title=" VCM 2023";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - vcm2023";
        }else{
            $this->salida->html_title="TEST - vcm2023";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                //$this->salida->manifiesto="vcm2023.manifest";
        ...
        $this->salida->agregar_js('dbo_vcm2023.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_vcm2023(), /// aca inserta los metadatos corriendo '..\operaciones_vcm2023\vcm2023_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_vcm2023\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_vcm2023\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_vcm2023();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'vcm2023',"),array(PRIMER_TLG,"'vcm2023',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_vcm2023::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_vcm2023\actualizar_a_vcm2023.sql'))));                    
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_vcm2023");
    ...
    */
    ...
    function proceso_recibir_paquete_descarga(){
        return new Proceso_recibir_paquete_descarga();
    }
    ...
    */    
    function proceso_grilla_vcm_I1_preguntas_abiertas(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Preguntas abiertas del I1',
            'permisos'=>array('grupo'=>'procesamiento','grupo1'=>'subcoor_campo'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'vcm_I1_preguntas_abiertas',
                array('pla_a1'=>'# =5|
                       pla_a1_esp!=NULL|pla_f8_15=1|pla_f8_especificar!=NULL|pla_f14g=1|pla_f14g_espe!=NULL|
                       pla_g2=1|pla_g3_13esp!=NULL|pla_g4o=1|pla_g4o_esp!=NULL|pla_g5_g=1|pla_g5_g_esp!=NULL|pla_g6_14=1|pla_g6_esp!=NULL|
                       pla_g8_7=1|pla_g8_7_esp!=NULL|pla_g9_17=1|pla_g9_17esp!=NULL|pla_observ !=NULL'                     
                ), 
                null,array());
            }
        ));
    ...
    */
    ...
    function proceso_grilla_i1_trabajo_ocupados_covid (){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'i1_trabajo_ocupados_covid',
                array(
                     's1_p_estado'=>'#>=77',
                ),
                null,array());
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_vcm2023());
}
?>
