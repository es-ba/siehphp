"use strict";
var version_js_seleccion_miembro='v 3.07';
...
window.addEventListener('load',function(){
    if(document.getElementById('var_tp') && operativo_actual=='vcm2023'){
...
function seleccionar_miembro(solo_controla,este){
    ...
    for(var num_miembro=1; num_miembro<=100; num_miembro++){    
        var pk_este=cambiandole(pk_ud,{tra_for:'S1', tra_mat:'P', tra_mie:num_miembro}); // queda:mie es una dbo
        ...
        if (ud_este){
            ...
            if ((letra_manual && uds_miembros[num_miembro].var_l0) || (!letra_manual && ud_este.var_edad>=18 && ud_este.var_edad<=999 && ud_este.var_sexo==2)){
        ...
        for(var cada_miembro_sorteado in en_rango){
            ...
            if(!letra_manual){
                if(solo_controla){                
                    ...
                    ud_este.var_l0=letra;
                    pk_este=cambiandole(pk_ud,{tra_for:'S1', tra_mat:'P', tra_mie:num_miembro_sorteado}); // queda:mie es una dbo
                    ...
                    if(!soy_un_ipad){
                        ...
                        var elemento_boton=document.getElementById('boton_S1_P_'+num_miembro_sorteado);
    ...
    if(solo_controla){
        ...
        if(grabar_si_es_necesario_o_seguir()){
            setTimeout(function(){
                ir_a_url('vcm2023.php?hacer=desplegar_formulario&todo={"tra_ope":"vcm2023","tra_for":"S1","tra_mat":""}');
            },200);
        }
    }
}
