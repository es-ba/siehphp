...
select comun.probar('create schema operaciones AUTHORIZATION tedede_php;');
...
where tem_enc in (select tem_enc from encu.tem)
  and tem_enc BETWEEN  100001 AND 999999;
*/

update encu.respuestas
  set res_valor=case tem_replica
                    when '1' then 1
                    when '5' then 3
                    when '6' then 3
                end
...
/*
select migrar_una_clave('eah2011','eah11_viv_s1a1','A1','','nenc','nhogar','miembro','null');
select migrar_una_clave('eah2011','eah11_ex','A1','X','nenc','nhogar','null','ex_miembro');
select migrar_una_clave('eah2011','eah11_i1','I1','','nenc','nhogar','miembro','null');
*/
