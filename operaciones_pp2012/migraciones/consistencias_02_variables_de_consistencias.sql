CREATE OR REPLACE VIEW encu.con_var_armada AS 
SELECT v.var_ope AS convar_ope, x.convar_con, x.vars[1] AS convar_var, 
        CASE
            ...
            ELSE v.var_texto
        END AS convar_texto, v.var_for AS convar_for, v.var_mat AS convar_mat, v.var_orden AS convar_orden
...
COALESCE(consistencias.con_postcondicion, ''::character varying)::text), '([A-Za-z][A-Za-z0-9_]*)(?![A-Za-z0-9_]|[.(])'::text, 'g'::text) AS vars
  ...
  WHERE x.vars[1] <> ALL (ARRAY['or'::text, 'and'::text]) and var_ope=dbo.ope_actual();
