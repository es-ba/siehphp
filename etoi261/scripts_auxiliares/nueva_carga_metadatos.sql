...
--tercer etapa agregar información de los metadatos de la eah2026
{{{
...
select dbo.ope_actual(), for_for, for_nombre, for_es_principal, for_orden, 1,
           for_tarea from operaciones_metadatos_eah2026.formularios;
/*OTRA*/ 
INSERT INTO encu.ua(
...
select dbo.ope_actual(), ua_ua, ua_prefijo_respuestas, ua_sufijo_tablas, ua_pk,  1
           from operaciones_metadatos_eah2026.ua;    
/*OTRA*/ 
...
select dbo.ope_actual(), mat_for, mat_mat, mat_texto, mat_ua, mat_ultimo_campo_pk, 
            mat_orden, mat_blanquear_clave_al_retroceder, 1  from operaciones_metadatos_eah2026.matrices;            
...
select dbo.ope_actual(), blo_for, blo_blo, blo_mat, blo_texto, blo_incluir_mat, 
            blo_orden, blo_aclaracion, 1 from operaciones_metadatos_eah2026.bloques;
/*OTRA*/            
INSERT INTO encu.con_opc(
            conopc_ope, conopc_conopc, conopc_texto, conopc_despliegue, conopc_tlg)
select dbo.ope_actual(), conopc_conopc, conopc_texto, conopc_despliegue, 1 from operaciones_metadatos_eah2026.con_opc;            
/*OTRA*/     
...
select dbo.ope_actual(), fil_for, fil_mat, fil_blo, fil_fil, fil_texto, fil_expresion, 
            fil_destino, fil_orden, fil_aclaracion, 1 from operaciones_metadatos_eah2026.filtros;
/*OTRA*/          
...
select dbo.ope_actual(), opc_conopc, opc_opc, opc_texto, opc_aclaracion, opc_orden, 
            opc_proxima_vacia, opc_proxima_opc, opc_proxima_texto, 1 from operaciones_metadatos_eah2026.opciones;
...
select dbo.ope_actual(), pre_pre, pre_texto, pre_abreviado, pre_for, pre_mat, 
            pre_blo, pre_aclaracion, pre_destino, pre_desp_opc, pre_desp_nombre, 
            pre_orden, 1, pre_aclaracion_superior from operaciones_metadatos_eah2026.preguntas;
/*OTRA*/
...
select dbo.ope_actual(), var_for, var_mat, var_pre, var_var, var_texto, var_aclaracion, 
            ...
            var_mejor_de_pregunta, var_maximo, var_minimo, var_advertencia_sup, 
            var_advertencia_inf, var_destino_nsnc, var_calculada, 
             1, var_nombre_dr from operaciones_metadatos_eah2026.variables;/*OTRA*/
/*OTRA*/
INSERT INTO encu.saltos(
            sal_ope, sal_var, sal_conopc, sal_opc, sal_destino, sal_tlg)
select dbo.ope_actual(), sal_var, sal_conopc, sal_opc, sal_destino, 1 from operaciones_metadatos_eah2026.saltos;
...
--select *   from  operaciones_metadatos_eah2026.variables
 -- from operaciones_metadatos_eah2026
...
select * 
  from encu.con_opc   
    --and   
    where conopc_ope=dbo.ope_actual() and conopc_conopc in  (select var_conopc from operaciones_metadatos_eah2026.variables where var_ope='eah2026' and  (var_for ='PMD' 
       or (  var_for='SUP' and var_pre in (select pre_pre from operaciones_metadatos_eah2026.preguntas where pre_blo='SPPMD' )) ))
)       ;
...
--select * 
 ...
 --2 casos
delete 
--select *  --from  operaciones_metadatos_eah2026.matrices
...
select * 
  from encu.con_opc   
    --and   
    where conopc_ope='eah2026' and conopc_conopc in  (select var_conopc from operaciones_metadatos_eah2026.variables where var_ope='eah2026' and  (var_for ='I1' 
       and var_pre in ( select pre_pre from operaciones_metadatos_eah2026.preguntas where pre_for ='I1'   and  pre_blo in ('MIG', 'SMM','DDHH' ) ) ))
)       ;
--con estos que me da esta consulta busco cuales no borrar
"dh1"
"dh2"
...
"m3"
select * from encu.con_opc
where conopc_conopc  in (select var_conopc  from operaciones_metadatos_eah2026.variables where var_ope='eah2026' and  var_conopc in (
'dh1',
'dh2',
's28',
'dh3',
