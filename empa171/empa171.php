...
$GLOBALS['anio_operativo']=2017;
//$GLOBALS['trimestre_operativo']=2;
$GLOBALS['esquema_principal']='encu';
$GLOBALS['titulo_corto_app']="EMPA 171";

$es_teletoi_desa = false;

$GLOBALS['es_tele_etoi']=strpos(__DIR__,"alserver_tele_etoi173/")>0 || !strpos(__DIR__,"alserver_etoi173/etoi173/")>0 && $es_teletoi_desa;
...
require_once "metadatos_empa171.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../empa171");

class Aplicacion_empa171 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EMPA 171";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - empa171";
        }else{
            $this->salida->html_title="TEST - empa171";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="empa171.manifest";
            }
        }
        ...
        $this->salida->agregar_js("../encuestas/control_mini_cal.js");
        ...
        $this->salida->agregar_js('dbo_empa171.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_empa171(), /// aca inserta los metadatos corriendo '..\operaciones_empa171\empa171_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                        ...
                        new Tabla_tem(),
                        new Tabla_glosario_tem(),
                        new Tabla_ocupacion(),
                        new Tabla_rama(),
                        new Tabla_parametros(),
                        new Tabla_import_info(),
                        new Tabla_valcan(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_empa171\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_empa171\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_empa171();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'empa171',"),array(PRIMER_TLG,"'empa171',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_empa171::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_empa171\actualizar_a_empa171.sql'))));                    
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_empa171");
    ...
    /*
    function proceso_desplegar_ue(){
        return $this->proceso_desplegar_formulario('UE');
    }
    */
    /*
    ...
    */
    ...
    function proceso_grilla_up_para_supervision(){
        return new Proceso_generico(array(
            ...
            'permisos'=>array('grupo'=>'mues_campo','grupo1'=>'procesamiento'),
            'submenu'=>'campo',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
    ...
    function proceso_grilla_muestrista_no_realizadas(){    
        return new Proceso_generico(array(
            ...
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'subcoor_campo','grupo3'=>'coor_listado'),
            'submenu'=>'muestrista',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
    ...
    function proceso_grilla_personal(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'submenu'=>'campo',
            'permisos'=>array('grupo'=>'subcoor_campo','grupo1'=>'ana_campo','grupo2'=>'coor_listado','grupo3'=>'procesamiento'),
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'personal', array('per_ope'=>$GLOBALS['NOMBRE_APP']));
            }
        ));
    }
    ...
    */    
    ...
    function proceso_grilla_registro_claves(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'submenu'=>'recepción',
            'permisos'=>array('grupo1'=>'recepcionista','grupo2'=>'procesamiento'),
            'funcion'=>function(Procesos $este){
    ...
    function proceso_grilla_glosario_tem(){
        return new Proceso_generico(array(
            'titulo'=>'Tabla Glosario de la TEM',
            'permisos'=>array(),
            'para_produccion'=>true,
            'submenu'=>'documentación',
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'glosario_tem', array());
            }
        ));
    }
    /*
    function proceso_grilla_GH_preg(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Preguntas del GH',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {           
               enviar_grilla($este->salida,'GH_proc',
                //array('pla_h2'=>'# =3|pla_h2=7'),null,array()); por ahora sin filtro 
                array(),null,array());                    
            }
        ));
    }
    ...
    */
    ...
    function proceso_grilla_ubi_geo(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Ubicación geográfica ',
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'cartografia'),
            'submenu'=>'procesamiento_EMPA',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {
               enviar_grilla($este->salida,'ubi_geo',
               array(),
               null,array());
            }
        ));
    }
    function proceso_grilla_hogar(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Hogar',
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'cartografia'),
            'submenu'=>'procesamiento_EMPA',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {
               enviar_grilla($este->salida,'hogar',
               array('pla_rel_ue'=>'# =2|pla_rel_ue<0'),
               null,array());
            }
        ));
    }
    function proceso_grilla_uni_eco(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Unidad Económica',
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'cartografia'),
            'submenu'=>'procesamiento_EMPA',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {
               enviar_grilla($este->salida,'uni_eco',
               array('pla_rel_ue'=>'# =1|pla_ue_1!null'),
               null,array());
            }
        ));
    }
    function proceso_grilla_uni_eco_docu(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Unidad Económica docu',
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'cartografia'),
            'submenu'=>'procesamiento_EMPA',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {
               enviar_grilla($este->salida,'uni_eco_docu',
               array('pla_rel_ue'=>'# =1|pla_ue_1!null'),
               null,array());
            }
        ));
    }    
    function proceso_grilla_rea_norea(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Rea-NoRea',
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'cartografia'),
            'submenu'=>'procesamiento_EMPA',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {
               enviar_grilla($este->salida,'rea_norea',
               array('pla_entrea'=>'#null|pla_entrea<=0|pla_razon1!null'),
               null,array());
            }
        ));
    }
    function proceso_grilla_documento(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Documento',
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'cartografia'),
            'submenu'=>'procesamiento_EMPA',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {
               enviar_grilla($este->salida,'documento',
               array('pla_rel_ue'=>'# =2|pla_rel_ue<0'),
               null,array());
            }
        ));
    }
    function proceso_grilla_personas(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Personas',
            'permisos'=>array('grupo'=>'procesamiento', 'grupo2'=>'cartografia'),
            'submenu'=>'procesamiento_EMPA',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este) {
               enviar_grilla($este->salida,'personas',
               array('pla_rel_ue'=>'# =2|pla_rel_ue<0'),
               null,array());
            }
        ));
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_empa171());
}
?>