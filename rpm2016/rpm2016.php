...
$GLOBALS['titulo_corto_app']="RPM 2016";

require_once "lo_imprescindible.php";
require_once "aplicaciones.php";
require_once "tabla_operativos.php";
require_once "metadatos_rpm2016.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../rpm2016");

class Aplicacion_rpm2016 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="RPM 2016";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - rpm2016";
        }else{
            $this->salida->html_title="TEST - rpm2016";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="rpm2016.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_rpm2016.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_rpm2016(), /// aca inserta los metadatos corriendo '..\operaciones_rpm2016\rpm2016_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_rpm2016\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_rpm2016\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_rpm2016();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'rpm2016',"),array(PRIMER_TLG,"'rpm2016',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_rpm2016::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_rpm2016\actualizar_a_rpm2016.sql'))));                    
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_rpm2016");
...
*/    
    ...
    function proceso_grilla_personal(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'submenu'=>'campo',
            'permisos'=>array('grupo'=>'subcoor_campo','grupo1'=>'ana_campo', 'grupo1'=>'ana_campo','grupo2'=>'coor_listado'),
            'funcion'=>function(Procesos $este){
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_rpm2016());
}
?>