...
require_once "metadatos_ebcp2014.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../ebcp2014");

global $revisando_metadatos;

$revisando_metadatos=true;

class Aplicacion_ebcp2014 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EBCP2014";
        }else{
            $this->salida->html_title="TEST - EBCP2014";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="ebcp2014.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_ebcp.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        "Tabla_result_sup",
                        "Tabla_verificado",
                        "Tabla_volver_a_cargar",                         
                    ) as $nombre_objeto_de_la_base)
                    {
                        $objeto_de_la_base=$este->nuevo_objeto($nombre_objeto_de_la_base);                        
                    ...
                    foreach(array(                  
                        '..\operaciones_ebcp2014\otras_sentencias_instalacion.sql',
                        //'..\operaciones_ebcp2014\insercion_tabla_tem.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_operativos.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_roles.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_rol_rol.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_usuarios.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_personal.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_ua.sql',
                        '..\operaciones_ebcp2014\ebcp2014_metadatos_tem.sql',
                        '..\operaciones_ebcp2014\ebcp2014_metadatos_produ_previos_instalacion.sql',                        
                        '..\operaciones_ebcp2014\insercion_tabla_estados_ingreso.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_importancia.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_tipo_nov.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_relaciones.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_planillas.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_a_ingreso.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_diccionario.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_dictra.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_dicvar.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_dispositivo.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_dominio.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_replica.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_result_sup.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_tabulados.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_verificado.sql',  
                        '..\operaciones_ebcp2014\insercion_tabla_volver_a_cargar.sql', 
                        //'..\operaciones_ebcp2014\insercion_tabla_formularios.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_estados.sql', 
                        '..\operaciones_ebcp2014\insercion_tabla_est_rol.sql',                               
                        '..\operaciones_ebcp2014\insercion_tabla_pla_est.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_est_var.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_var_cal.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_varcalopc.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_pla_var.sql',  
                        '..\operaciones_ebcp2014\vista_var_orden.sql',
                        '..\operaciones_ebcp2014\insercion_tabla_consistencias.sql',
                        '..\operaciones\trigger_sincronizacion_tem_datos.sql',
                        ) as $archivo)
                    {
                        $sentencias_generales=explode("/*OTRA*/",file_get_contents($archivo));
                        $sql_sentencias=str_replace(array('/*CAMPOS_AUDITORIA*/',"'eah2013',"),array(','.PRIMER_TLG,"'etoi143',"),$sentencias_generales);
                        if(!$esta_es_la_base_en_produccion){
                            $este->db->beginTransaction();
                    ...
                    /*
                    $consistencias=explode("INSERT INTO consistencias",file_get_contents("../operaciones_ebcp2014/consistencias_ebcp2014.sql"));
                    ...
                    */
                    foreach(array(                    
                                  'funciones_dbo.sql',
                                  '..\encuestas\generar_consistencias_flujo.sql',
                                  '..\encuestas\generar_consistencias_audi_nsnc.sql',                                  
                                  '..\encuestas\generar_consistencias_audi_rango.sql',                                  
                                 ) as $archivo2 )
                    {
                        $funciones_generales=explode("/*otra*/",file_get_contents($archivo2));
                        $este->salida->enviar('instalando funcion '.$archivo2);
                        foreach($funciones_generales as $cada_funcion) {                            
                    ...
                    $estructura_encuesta = new Estructura_ebcp2014();
                    $estructura_encuesta->contexto=$este;
                    $estructura_encuesta->generar_estructura();
                    
                    //$archivo='..\operaciones_ebcp2014\insercion_tabla_consistencias.sql';
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'AJUS',"),array(','.PRIMER_TLG,"'ebcp2014',"),file_get_contents($archivo))));
    ...
    function proceso_actualizar_instalacion(){
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                global $esta_es_la_base_en_produccion;
                try{
                    Aplicacion_ebcp2014::salida_enviar_aviso_instalacion($este);
                    ...
                    foreach(array(
                        '..\operaciones_ebcp2014\actualizar_a_ebcp2014.sql',
                        '..\encuestas\trigger_respuestas_a_planas_trg.sql',
                        '..\ebcp2014\trigger_calculo_estado_trg.sql',// se pone en la carpeta ebcp porque en encuestas queda el original y en ebcp hay que hacer un cambio específico
                    ) as $nombre_script){
                        $este->salida->enviar('corriendo '.$nombre_script);
                        $este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents($nombre_script))));
                    }
                    $este->salida->enviar('Estructura');
                    $estructura_encuesta = new Estructura_ebcp2014();
                    $estructura_encuesta->contexto=$este;
                    $estructura_encuesta->generar_estructura();
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $tabla_respuestas=$este->nuevo_objeto('Tabla_respuestas');
                    $este->db->ejecutar_sqls($tabla_respuestas->restricciones_especificas());
                    $este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_ebcp2014\actualizar_a_ebcp2014.sql'))));
                    $estructura_encuesta = new Estructura_ebcp2014();
                    $estructura_encuesta->contexto=$este;
                    $estructura_encuesta->generar_estructura();
                    if($esta_es_la_base_en_produccion){
...
JS
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_ebcp2014");
    ...
    function proceso_grilla_con_opc(){
        return new Proceso_generico(array(
            'titulo'=>'Tabla con opciones',
            'permisos'=>array('grupo'=>'subcoor_campo','grupo2'=>'procesamiento','grupo3'=>'metadatos'),
            'para_produccion'=>true,
            'submenu'=>'metadatos',
            'funcion'=>function(Procesos $este){
...
/*    function proceso_planilla_recepcion_recuperador(){
    ...
    function proceso_grilla_control_muestra(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Control de muestra',
    ...
    function proceso_discapacidad_y_rol_de_genero(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de preguntas abiertas: Discapacidad y rol de género',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'discapacidad_y_rol_de_genero');
            }
        ));
    }
    function proceso_I1_educacion(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de preguntas abiertas: Educación',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_educacion');
            }
        ));
    }
    function proceso_I1_sm(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de preguntas abiertas: SM',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_sm');
            }
        ));
    }
    function proceso_I1_sm_mac_em(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de preguntas abiertas SM - MAC EM',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_sm_mac_em');
            }
        ));
    }
    function proceso_I1_tip(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla: TIP - Transmisión intergeneracional de la pobreza',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_tip');
            }
        ));
    }
    function proceso_I1_vida_cotidiana(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de preguntas abiertas: Vida cotidiana',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_vida_cotidiana');
            }
        ));
    }
    ...
    function proceso_bienvenida(){
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este){
                global $soy_un_ipad;
                $este->salida->enviar('Pantalla de entrada al sistema en producción de la EBCP','',array('tipo'=>'h2'));
    ...
    */
    function proceso_grilla_A1(){
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este) {           
               ...
               enviar_grilla($este->salida,'A1',array('pla_v2'=>'# =8|pla_v5=4|pla_h2b=7|pla_h20_14=1'),null,array());                
            }
        ));
    }
    function proceso_grilla_I1_trabajo(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla de Preguntas abiertas del I1 trabajo',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_trabajo_ebcp',array('pla_t8'=>'# =3|pla_t11=4|pla_t14b=6|pla_t15a=1|pla_t23!=NULL|pla_t24!=NULL|pla_t25!=NULL|pla_t26!=NULL|pla_t36_7=1|pla_t36_8=1|pla_t37!=NULL|pla_t38b=8|pla_t39=4|pla_t41!=NULL|pla_t42!=NULL|pla_t43!=NULL'),null,array());
            }
        ));
    }
    function proceso_grilla_I1_ingresos(){
        return new Proceso_generico(array(
            'titulo'=>'Grilla Consistencia Ingresos en I1',
            'permisos'=>array('grupo'=>'procesamiento'),
            'submenu'=>'procesamiento',
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_ingresos_ebcp',array(),null,array());
            }
        ));
    }
    ...
    function proceso_grilla_A1_X_(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'A1_X_',array(),null,array());
            }
        ));
    }
    ...
    function proceso_grilla_I1_salud(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_salud',array('pla_sn1_1'=>'#=1|pla_sn1_7=1|pla_sn1_2=1|pla_sn1_3=1|pla_sn1_4=1|pla_sn2=1|pla_sn4=4|pla_sn5=9|pla_sn13=5|pla_sn14=4'),null,array());
            }
        ));
    }
    function proceso_grilla_tabulados(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'tabulados',null,null,array());
            }
        ));
    }
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_ebcp2014());
}
?>