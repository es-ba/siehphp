<?php
//UTF-8:SÍ
class Para_Pruebas_Procesos_con_TEM__ebcp2014 extends Pruebas{
    // GRANT CREATE ON DATABASE ebcp2014_copia_db TO tedede_php;
    var $ope_prueba;
...
class Prueba_Proceso_cargar_dispositivo__ebcp2014 extends Para_Pruebas_Procesos_con_TEM__ebcp2014{
    var $tem_tipico=array('dominio'=>1,'replica'=>1, 'comuna'=>1, 'up'=>-11, 'lote'=>2, 'reserva'=>1);
    function probar_liberar_encuestas(){
        $this->cargar_tem(1001,null,array('up'=>'-11', 'reserva'=>'1'));
    ...
    function probar_asignar_dos_encuestas_a_un_encuestador_por_primera_vez(){
        foreach(
            array(
    ...
    function probar_cargar_dos_encuestas_a_un_encuestador_por_primera_vez(){
        global $hoy;
        foreach(
            array(
    ...
    /*function probar_fin_descarga_enc(){
    ...
    function probar_mandar_a_ingreso_enc(){
        global $hoy;
        foreach(
            array(
    ...
    function probar_volver_a_cargar_enc(){
        global $hoy;
        foreach(
            array(
            ...
            $this->cargar_tem(1002,array('asignable'=>1,'cod_enc'=>-1,'dispositivo_enc'=>2,'fecha_carga_enc'=>'2013-09-13','fecha_descarga_enc'=>'2013-09-13','con_dato_enc'=>$caso['con_dato']),$this->tem_tipico);
            $this->verificar_tem(1002,array('estado'=>$caso['estado_ini']),"en caso {$id_caso}");
            ...
            $this->verificar_tem(1002,array('estado'=>24,'fecha_descarga_enc'=>null,'con_dato_enc'=>null,'volver_a_cargar_enc'=>null),"en caso {$id_caso}");
    ...
    /*function probar_volver_a_cargar_recu(){
        global $hoy;
        foreach(
            array(
    ...
    function x_probar_tem_desde_archivo(){
        ...
        $contenido=file_get_contents('..\..\pruebas_automaticas\campo\casos_TEM_ebp2014.txt');
        $numero_linea=0;
        $this->tabla_plana_tem=$this->contexto->nuevo_objeto("Tabla_plana_TEM_");
        $this->tabla_tem=$this->contexto->nuevo_objeto("Tabla_TEM");
        foreach(explode("\n",$contenido) as $linea){
    ...
    function probar_deshacer_carga(){
