...
DECLARE v_limite_anio_inf integer;
DECLARE v_limite_anio_sup integer;
BEGIN
  if p_valor is null then
    return null;
  end if;
  v_anio_actual := dbo.anio();
  v_limite_anio_inf:=substring(v_anio_actual::text from 3 for 4)::integer;
  v_limite_anio_sup:=v_limite_anio_inf+1;
  ...
  if v_anio_extraido = -1 then
    ...
    if v_anio_extraido < 100 and v_anio_extraido > v_limite_anio_inf then 
      v_anio_extraido := v_anio_extraido + 1900;
      v_fecha := to_date(v_dia_extraido || '/' || v_mes_extraido || '/' || v_anio_extraido, 'DD/MM/YYYY');
    elsif v_anio_extraido < v_limite_anio_sup then 
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
CREATE OR REPLACE FUNCTION dbo.max_hog_ingresado(p_enc integer)
  RETURNS integer AS
$BODY$
DECLARE
 v_cantidad integer;
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
CREATE OR REPLACE FUNCTION dbo.cant_S1p_x_hog(p_enc integer, p_hog integer)
  RETURNS integer AS
$BODY$
DECLARE
 v_cantidad integer;
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
CREATE OR REPLACE FUNCTION dbo.tit_mujer_madre_menor_5_anios(p_enc integer, p_hog integer)
  RETURNS boolean AS
$BODY$
DECLARE
 v_cantidad integer;
 v_titular integer;
BEGIN 
  select s1.pla_nmiembro_titular into v_titular
    from encu.plana_s1_ s1  inner join encu.plana_s1_p s1p on s1.pla_enc=s1p.pla_enc and s1.pla_hog=s1p.pla_hog 
    where s1.pla_enc=p_enc and s1.pla_hog=p_hog and s1.pla_nmiembro_titular=s1p.pla_p4 and s1p.pla_sexo=2; 

  select count(*) into v_cantidad
    from encu.plana_s1_p where pla_enc=p_enc and pla_hog=p_hog 
    and pla_edad<5 and pla_p6_b=v_titular;
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
ALTER FUNCTION dbo.tit_mujer_madre_menor_5_anios(integer, integer)
  OWNER TO tedede_php;
/*otra*/
CREATE OR REPLACE FUNCTION dbo.meses_a_la_fecha(p_f_nac text, p_f_realiz text)
  RETURNS integer AS
$BODY$
DECLARE v_anios integer;
DECLARE v_meses integer;
DECLARE v_meses_parciales integer;
BEGIN
  if dbo.texto_a_fecha(p_f_nac) > dbo.texto_a_fecha(p_f_realiz) then 
    return null;
  end if;
  v_anios=extract(years from age(dbo.texto_a_fecha(p_f_realiz),dbo.texto_a_fecha(p_f_nac)));
  v_meses_parciales=extract(mons from age(dbo.texto_a_fecha(p_f_realiz),dbo.texto_a_fecha(p_f_nac)));
  v_meses=v_anios*12+v_meses_parciales;
  if v_meses>1560 or v_meses<0 then
    return null;
  end if;
  return v_meses;
EXCEPTION
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/  
ALTER FUNCTION dbo.meses_a_la_fecha(text, text)
  OWNER TO tedede_php;
/*otra*/
CREATE OR REPLACE FUNCTION dbo.tit_mujer_madre_mayor_8_meses(p_enc integer, p_hog integer)
  RETURNS boolean AS
$BODY$
DECLARE
 v_cantidad integer;
...
BEGIN 
  ...
  select count(*) into v_cantidad
    from encu.plana_s1_ s1 inner join encu.plana_s1_p s1p on s1.pla_enc=s1p.pla_enc and s1.pla_hog=s1p.pla_hog
    where s1.pla_enc=p_enc and s1.pla_hog=p_hog and dbo.meses_a_la_fecha(pla_f_nac_o, pla_f_realiz_o)>8 and s1p.pla_p6_b=v_titular;
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
ALTER FUNCTION dbo.tit_mujer_madre_mayor_8_meses(integer, integer)
  OWNER TO tedede_php;
/*otra*/
CREATE OR REPLACE FUNCTION dbo.tiene_hijo(p_enc integer, p_hog integer, p_mie integer)
  RETURNS boolean AS
$BODY$
DECLARE
 v_cantidad integer; 
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
ALTER FUNCTION dbo.tiene_hijo(integer, integer, integer)
  OWNER TO tedede_php;
/*otra*/  
CREATE OR REPLACE FUNCTION dbo.menor_de_45_dias_a_4_anios(p_enc integer, p_hog integer)
  RETURNS boolean AS
$BODY$
DECLARE
 v_cantidad integer;
...
BEGIN 
    ...
    select count(*) into v_cantidad
    from encu.plana_s1_ s1 inner join encu.plana_s1_p s1p on s1.pla_enc=s1p.pla_enc and s1.pla_hog=s1p.pla_hog
    where s1.pla_enc=p_enc and s1.pla_hog=p_hog and dbo.dias_a_la_fecha(pla_f_nac_o, pla_f_realiz_o)>44 
    and dbo.edad_a_la_fecha(pla_f_nac_o, pla_f_realiz_o)<5;
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
 ALTER FUNCTION dbo.menor_de_45_dias_a_4_anios(integer, integer)
  OWNER TO tedede_php;  
/*otra*/  
CREATE OR REPLACE FUNCTION dbo.dias_a_la_fecha(p_f_nac text, p_f_realiz text)
  RETURNS integer AS
$BODY$
...
BEGIN
  ...
  v_dias:=dbo.texto_a_fecha(p_f_realiz)-dbo.texto_a_fecha(p_f_nac);
  if v_dias<0 then
    return null;
  end if;
  return v_dias;
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
ALTER FUNCTION dbo.dias_a_la_fecha(text, text)
  OWNER TO tedede_php;
/*otra*/
CREATE OR REPLACE FUNCTION comun.es_cadena_vacia(ptexto text)
  RETURNS boolean AS
$$  
    select (length(ptexto)=0) is true ; 
$$  LANGUAGE sql IMMUTABLE;
/*otra*/
CREATE OR REPLACE FUNCTION dbo.anio(p_valor text)
  RETURNS integer AS
$BODY$
DECLARE v_anio_extraido integer;
DECLARE v_anio_actual integer;
...
BEGIN
  ...
  return v_anio_extraido::integer;
EXCEPTION
  WHEN invalid_datetime_format THEN
    return null;
  WHEN datetime_field_overflow THEN
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
ALTER FUNCTION dbo.anio(text)
  OWNER TO tedede_php;
/*otra*/
ALTER FUNCTION comun.es_cadena_vacia(text)
  OWNER TO tedede_php;
/*otra*/
CREATE OR REPLACE FUNCTION comun.ignorado(valor text)
  RETURNS boolean AS
$BODY$
BEGIN
  IF valor = '--' THEN
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
ALTER FUNCTION comun.ignorado(text)
  OWNER TO tedede_php;
/*otra*/
CREATE OR REPLACE FUNCTION comun.ignorado(valor integer)
  RETURNS boolean AS
$BODY$
BEGIN
  IF valor = -1 THEN
...
$BODY$
  LANGUAGE plpgsql IMMUTABLE;
/*otra*/
ALTER FUNCTION comun.ignorado(integer)
  OWNER TO tedede_php;
