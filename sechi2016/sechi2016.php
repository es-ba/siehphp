...
class Aplicacion_sechi2016 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="SECHI 2016";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - sechi2016";
        }else{
            $this->salida->html_title="TEST - sechi2016";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="sechi2016.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_sechi2016.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_sechi2016(), /// aca inserta los metadatos corriendo '..\operaciones_sechi2016\sechi2016_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_sechi2016\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_sechi2016\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                                   ...
                                   ) as $archivo)
                    {
                        ...
                        $sql_sentencias=str_replace(array('/*CAMPOS_AUDITORIA*/',"'eah2015',"),array(PRIMER_TLG,"'dbo.ope_actual()',"),$sentencias_generales);
                        if(!$esta_es_la_base_en_produccion){
                            $este->db->beginTransaction();
                    ...
                    $estructura_encuesta = new Estructura_sechi2016();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'sechi2016',"),array(PRIMER_TLG,"'sechi2016',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_sechi2016::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_sechi2016\actualizar_a_sechi2016.sql'))));                    
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_sechi2016");
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_sechi2016());
}
?>