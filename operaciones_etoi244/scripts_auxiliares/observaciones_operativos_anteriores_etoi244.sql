...
drop table if exists operaciones_ope_claves_respuestas.claves242;
drop table if exists operaciones_ope_claves_respuestas.respuestas242;
---claves y respuestas
set role tedede_php;
--SELECT 
...
select * into operaciones_ope_ant.variableseah2024
  from encu.variables
  where var_ope='eah2024' and var_for in ('A1', 'S1')
  order by var_var;
  
 */
--para eah2024 tomo los metadatos desde operaciones_metadatos porque ya estàn cargados
 
  
----carga de metadatos

update encu.operativos set ope_ope_anterior='eah2024' where ope_ope='etoi244';
...
select blo_ope, blo_for, blo_blo, blo_mat, blo_texto, blo_incluir_mat, 
    ...
    where blo_ope ='etoi242' and blo_for in ('A1', 'S1')
union 
select blo_ope, blo_for, blo_blo, blo_mat, blo_texto, blo_incluir_mat, 
            blo_orden, blo_aclaracion, 1 
...
select pre_ope, pre_pre, pre_texto, pre_abreviado, pre_for, pre_mat, 
    ...
    where pre_ope ='etoi242' and pre_for in ('A1', 'S1')
union
select pre_ope, pre_pre, pre_texto, pre_abreviado, pre_for, pre_mat, 
            pre_blo, pre_aclaracion, pre_destino, pre_desp_opc, pre_desp_nombre, 
            pre_orden, pre_aclaracion_superior, 1 
...
select      var_ope, var_for, var_mat, var_pre, var_var, var_texto, var_aclaracion, 
    ...
    where var_ope ='etoi242' and var_for in ('A1', 'S1')
 union
select      var_ope, var_for, var_mat, var_pre, var_var, var_texto, var_aclaracion, 
            var_conopc, var_conopc_texto, var_tipovar, var_destino, var_subordinada_var, 
            var_subordinada_opcion, var_desp_nombre, var_expresion_habilitar, 
...
select pla_participacion, pla_rotaci_n_eah, pla_rotaci_n_etoi, count(*)  
  ...
  where pla_dominio=3 and pla_participacion in (2,3)
    and pla_semana  between 1 and 5
...
--select distinct  cla_ope,  cla_enc --360
    from operaciones_ope_claves_respuestas.claves242
    where cla_enc in (select pla_enc from encu.plana_tem_  
                          where pla_dominio=3 and pla_participacion in (3)
    and pla_semana  between 1 and 5)
        and cla_ope in ('etoi242') and cla_for in ('A1', 'S1')
union
select cla_ope, cla_for, cla_mat, cla_enc, 
       cla_hog, cla_mie, cla_exm, 1
--select distinct  cla_ope,  cla_enc -800
...
--select distinct  res_ope,  res_enc -- 360
    from operaciones_ope_claves_respuestas.respuestas242
    where res_enc in (select pla_enc from encu.plana_tem_  
                          where pla_dominio=3 and pla_participacion in (3)
    and pla_semana  between 1 and 5)
        and res_ope in ('etoi242') and res_for in ('A1', 'S1')
union
select res_ope, res_for, res_mat, res_enc, res_hog, res_mie, res_exm, --
       res_var, res_valor, res_valesp, res_valor_con_error, res_estado, 
       res_anotaciones_marginales, 1         
--select distinct  res_ope,  res_enc --  800
...
--comprobacion
select res_ope,res_enc, count(*) -- 1160 filas 800+360
from encu.respuestas
where res_ope <> 'etoi244' and res_enc 
in (select pla_enc from encu.plana_tem_  where pla_dominio=3 --and  pla_participacion=2
    and pla_semana  between 1 and 6)
group by 1,2
order by 1,2;

select cla_ope,cla_enc, count(*) ----1160 filas
from encu.claves
where cla_ope <> 'etoi244'
...
--,count(*) --filas --1166 filas
from encu.respuestas
where res_ope <> 'etoi244' and res_var='s1a1_obs' --and res_hog >1
--group by 1,2,3,4,5
--having count(*) >1
...
-----
----carga de semanas 6 a 12
 --casos
set role tedede_php;
select pla_participacion, pla_rotaci_n_eah, pla_rotaci_n_etoi, count(*)  
  from encu.plana_tem_   
  where pla_dominio=3 and pla_participacion in (2,3)
    and pla_semana between 6 and 12
  group by pla_participacion, pla_rotaci_n_eah, pla_rotaci_n_etoi
  order by pla_participacion;
--2    1    1    560
--3    1    3    640  
...
select cla_ope, cla_for, cla_mat, cla_enc, 
       cla_hog, cla_mie, cla_exm, 1
--select distinct  cla_ope,  cla_enc --640
    from operaciones_ope_claves_respuestas.claves242
    where cla_enc in (select pla_enc from encu.plana_tem_  
                          where pla_dominio=3 and pla_participacion in (3)
    and pla_semana  between 6 and 12)
...
select cla_ope, cla_for, cla_mat, cla_enc, 
       cla_hog, cla_mie, cla_exm, 1
--select distinct  cla_ope,  cla_enc --1200
    from operaciones_ope_claves_respuestas.claves2024
    where cla_enc in (select pla_enc from encu.plana_tem_  
                          where pla_dominio=3 and pla_participacion in (2,3)  and pla_rotaci_n_eah=1
    and pla_semana  between 6 and 12)
...
select res_ope, res_for, res_mat, res_enc, res_hog, res_mie, res_exm, --
       res_var, res_valor, res_valesp, res_valor_con_error, res_estado, 
       res_anotaciones_marginales, 1         
--select distinct  res_ope,  res_enc --  640 casos
    from operaciones_ope_claves_respuestas.respuestas242
    where res_enc in (select pla_enc from encu.plana_tem_  
                          where pla_dominio=3 and pla_participacion in (3)
    and pla_semana  between 6 and 12)
...
select res_ope, res_for, res_mat, res_enc, res_hog, res_mie, res_exm, --
       res_var, res_valor, res_valesp, res_valor_con_error, res_estado, 
       res_anotaciones_marginales, 1         
--select distinct  res_ope,  res_enc -- 1200 
    from operaciones_ope_claves_respuestas.respuestas2024
    where res_enc in (select pla_enc from encu.plana_tem_  
                          where pla_dominio=3 and pla_participacion in (2,3)  and pla_rotaci_n_eah=1
    and pla_semana  between 6 and 12)
...
alter table encu.claves enable trigger claves_ins_trg; 
 ...
 --comprobacion
select res_ope,res_enc, count(*) --  1840 filas = (640 partic=3)+(1200 part2+3)
from encu.respuestas
where res_ope <> 'etoi244' and res_enc 
in (select pla_enc from encu.plana_tem_  where pla_dominio=3 --and  pla_participacion=2
    and pla_semana  between 6 and 12)
...
select cla_ope,cla_enc, count(*) ----1840  filas = (640 partic=3)+(1200 part2+3)
from encu.claves
where cla_ope <> 'etoi244'
and cla_enc in (select pla_enc from encu.plana_tem_  where pla_dominio=3  --and pla_participacion=2
    and pla_semana  between 6 and 12)
...
from encu.respuestas
where res_ope <> 'etoi244' and res_var='s1a1_obs' --and res_hog >1
and res_enc in (select pla_enc from encu.plana_tem_ where pla_semana between 6 and 12)
--group by 1,2,3,4,5
