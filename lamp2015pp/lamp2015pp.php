...
$GLOBALS['titulo_corto_app']="LAMP 2015 PP";

require_once "lo_imprescindible.php";
require_once "aplicaciones.php";
require_once "tabla_operativos.php";
require_once "metadatos_lamp2015pp.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../lamp2015pp");


class Aplicacion_lamp2015pp extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="LAMP 2015 Prueba Piloto";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - lamp2015pp Prueba Piloto";
        }else{
            $this->salida->html_title="TEST - lamp2015pp Prueba Piloto";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="lamp2015pp.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_lamp2015pp.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_lamp2015pp\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_lamp2015pp\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                                   ...
                                   ) as $archivo)
                    {
                        ...
                        $sql_sentencias=str_replace(array('/*CAMPOS_AUDITORIA*/',"'etoi151',"),array(PRIMER_TLG,"'dbo.ope_actual()',"),$sentencias_generales);
                        if(!$esta_es_la_base_en_produccion){
                            $este->db->beginTransaction();
                        ...
                        $este->salida->enviar('ya instalado '.$archivo);                        
                        if(!$esta_es_la_base_en_produccion){
                    ...
                    //ML ojo, estaba activado, ver con graciela
                    ...
                    $estructura_encuesta = new Estructura_lamp2015pp();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'lamp2015pp',"),array(PRIMER_TLG,"'lamp2015pp',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_lamp2015pp::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_lamp2015pp\actualizar_a_lamp2015pp.sql'))));                    
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_lamp2015pp");
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_lamp2015pp());
}
?>