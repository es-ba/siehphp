...
class Aplicacion_eah2013 extends Aplicacion_encuesta{
    function __construct(){
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="EAH2013";
        }else{
            $this->salida->html_title="TEST - EAH2013";
        }
        $this->salida->color_fondo=(@$GLOBALS['color_de_fondo_de_la_aplicacion'])?:'#FDE'; // NO CAMBIAR ACÁ EL COLOR ESTE ES EL ROSA DE PRUEBA
        $this->salida->img_fondo="eah_fondo_a2.png); background-position: bottom; background-attachment: fixed; background-repeat: repeat-x; other:(";
        (@$GLOBALS['color_de_fondo_de_la_aplicacion'])?:'#FDE'; // NO CAMBIAR ACÁ EL COLOR ESTE ES EL ROSA DE PRUEBA
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="eah2013.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_eah.js');
    }
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array('..\operaciones_eah2013\otras_sentencias_instalacion.sql',
                                  '..\operaciones_eah2013\insercion_tabla_tem.sql',
                                  '..\operaciones_eah2013\insercion_tabla_roles.sql',
                                  '..\operaciones_eah2013\insercion_tabla_rol_rol.sql',
                                  '..\operaciones_eah2013\insercion_tabla_usuarios.sql',
                                  //'..\operaciones_eah2013\insercion_tabla_usuarios_nuevos.sql',
                                  '..\operaciones_eah2013\insercion_tabla_personal.sql',
                                  '..\operaciones_eah2013\insercion_tabla_estados_ingreso.sql',
                                  '..\operaciones_eah2013\insercion_tabla_importancia.sql',                                                                    
                                  '..\operaciones_eah2013\insercion_tabla_tipo_nov.sql',
                                  '..\operaciones_eah2013\insercion_tabla_relaciones.sql',  
                                  '..\operaciones_eah2013\insercion_tabla_planillas.sql',  
                                  //'..\operaciones_eah2013\insercion_tabla_pla_var.sql',  
                                  'vista_var_orden.sql',
                                   ) as $archivo)
                    {
                        $este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'AJUS',"),array(','.PRIMER_TLG,"'eah2013',"),file_get_contents($archivo))));
                    }
                    /*
                    $consistencias=explode("INSERT INTO consistencias",file_get_contents("../operaciones_eah2013/consistencias_eah2013.sql"));
                    ...
                    */
                    ...
                    $estructura_encuesta = new Estructura_eah2013();
                    $estructura_encuesta->contexto=$este;
                    $estructura_encuesta->generar_estructura();
                    $archivo='..\operaciones_eah2013\insercion_tabla_consistencias.sql';
                    $este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'AJUS',"),array(','.PRIMER_TLG,"'eah2013',"),file_get_contents($archivo))));
                    if($esta_es_la_base_en_produccion){
                        $este->db->commit();
                    }
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_eah2013::salida_enviar_aviso_instalacion($este);
                if($mas_planas){
                    foreach(array(
                        new Tablas_planas(),
                    ) as $objeto_de_la_base)
            ...
            $este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('../operaciones_eah2013/actualizar_a_eah2013.sql'))));
            $estructura_encuesta = new Estructura_eah2013();
            $estructura_encuesta->contexto=$este;
            $estructura_encuesta->generar_estructura();
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_eah2013");
    ...
    function proceso_formularios_de_la_vivienda(){
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                $para_supervisor=@$_COOKIE['para_supervisor']?'1':'0';
...
JS
    ...
    function proceso_grilla_A1(){
        return new Proceso_generico(array(
            ...
            'funcion'=>function(Procesos $este) {           
               ...
               enviar_grilla($este->salida,'A1',array('pla_v2'=>'# =8|pla_v5=4|pla_h2=7|pla_h20_14=1'),null,array());                
            }
        ));
    }
    function proceso_grilla_I1_trabajo_ingresos(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_trabajo_ingresos',array('pla_t8'=>'# =3|pla_t11=4|pla_t23!=NULL|pla_t24!=NULL|pla_t25!=NULL|pla_t26!=NULL|pla_t37!=NULL|pla_t39=4|pla_t39_bis2=8|pla_t41!=NULL|pla_t42!=NULL|pla_t43!=NULL|pla_i3_10=1'),null,array());
            }
        ));
    }
    ...
    function proceso_grilla_I1_salud(){
        return new Proceso_generico(array(
            ...
            'para_produccion'=>true,
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'I1_salud',array('pla_sn1_1'=>'#=1|pla_sn1_7=1|pla_sn1_2=1|pla_sn1_3=1|pla_sn1_4=1|pla_sn2=1|pla_sn4=4|pla_sn5=9|pla_sn12=2|pla_sn13=5|pla_sn14=4|pla_sn15k=1'),null,array());
            }
        ));
    }
    ...
    /*
    function proceso_grilla_baspro(){
        return new Proceso_generico(array(
            ...
            'submenu'=>'procesamiento',
            'funcion'=>function(Procesos $este){
                enviar_grilla($este->salida,'baspro',array(),false,array('otras_opciones'=>array('agregando_filas_completas'=>true)));
            }
        ));
    }
    */
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_eah2013());
}
?>