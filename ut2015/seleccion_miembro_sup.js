"use strict";
var version_js_seleccion_miembro_sup='V 2.29';
...
window.addEventListener('load',function(){
    if(document.getElementById('var_sut_tp') && operativo_actual=='ut2015'){
        var padre=document.getElementById('var_sut_tp').parentNode;
...
function seleccionar_miembro_sup(solo_controla,este){
    ...
    for(var num_miembro=1; num_miembro<=100; num_miembro++){    
        ...
        if (ud_este){
            ...
            if ((letra_manual && uds_miembros[num_miembro].var_sut_l0) || (!letra_manual && ud_este.var_sut_edad>=14 && ud_este.var_sut_edad<=999)){
                cant_candidatos=cant_candidatos+1;
                en_rango.push({
                    edad:ud_este.var_sut_edad, 
                ...
                uds_miembros[num_miembro].var_sut_l0=null;                
    ...
    if(cant_candidatos==0){
        if(solo_controla){
            control=(
                ...
                !rta_ud.var_sut_cr_num_miembro &&
                rta_ud.var_sut_tp==0
            );
        }else{
            // rta_ud.var_cr_ningun_miembro=1;
            rta_ud.var_sut_cr_num_miembro=null;
            rta_ud.var_sut_tp=0;
        }
    }else{
        ...
        for(var cada_miembro_sorteado in en_rango){
            ...
            if(!letra_manual){
                if(solo_controla){                
                    control=control && uds_miembros[num_miembro_sorteado].var_sut_l0==letra;                
                }else{                
                    var ud_este=uds_miembros[num_miembro_sorteado];
                    ud_este.var_sut_l0=letra;
        ...
        if(solo_controla){
            control=control && (
                ...
                rta_ud.var_sut_cr_num_miembro==elegido &&
                rta_ud.var_sut_tp==cant_candidatos
            );
        }else{
    ...
    if(solo_controla){
        ...
        if(grabar_si_es_necesario_o_seguir()){
            setTimeout(function(){
                ir_a_url('ut2015.php?hacer=desplegar_formulario&todo={"tra_ope":"ut2015","tra_for":"SUP","tra_mat":""}');
            },200);
        }
    }
}
