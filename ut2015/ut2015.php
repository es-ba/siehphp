...
$GLOBALS['titulo_corto_app']="UT 2015";

require_once "lo_imprescindible.php";
require_once "aplicaciones.php";
require_once "tabla_operativos.php";
require_once "metadatos_ut2015.php";
require_once "todos_los_php.php";
incluir_todo("../tedede");
incluir_todo("../encuestas");
incluir_todo("../ut2015");

class Aplicacion_ut2015 extends Aplicacion_encuesta{
    function __construct(){
        global $esta_es_la_base_en_produccion,$esta_es_la_base_de_capacitacion,$soy_un_ipad,$esta_es_la_base_de_supervision;
        ...
        if($esta_es_la_base_en_produccion){
            $this->salida->html_title="UT 2015";
        }else if($esta_es_la_base_de_capacitacion){
            $this->salida->html_title="CAPA - ut2015";
        }else if($esta_es_la_base_de_supervision){
            $this->salida->html_title="SUPERVISION - ut2015";    
        }else{
            $this->salida->html_title="TEST - ut2015";
        }
        if($soy_un_ipad){
            if(isset($_REQUEST['hacer'])&&isset($this->ver_offline[$_REQUEST['hacer']])){
                $this->salida->manifiesto="ut2015.manifest";
            }
        }
        ...
        $this->salida->agregar_js('dbo_ut2015.js');
    }
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    foreach(array(
                        ...
                        //new Metadatos_ut2015(), /// aca inserta los metadatos corriendo '..\operaciones_ut2015\ut2015_dump.json'
                        new Tablas_planas(),
                        new Tabla_tabulados(),
                    ...
                    foreach(array('..\operaciones\insercion_otras_sentencias_instalacion.sql',//ok 
                                        //'..\operaciones_ut2015\actualizacion_ope_y_anio.sql', //ATENCION: se corre a mano la actualización (#1899)
                                  '..\operaciones\insercion_tabla_http_user_agent.sql',
                                  '..\operaciones_ut2015\insercion_tabla_tem.sql',
...
//                                  '..\operaciones\insercion_tabla_claves.sql',
                    ...
                    $estructura_encuesta = new Estructura_ut2015();
                    ...
                    //$este->db->ejecutar_sql(new Sql(str_replace(array('/*CAMPOS_AUDITORIA*/',"'ut2015',"),array(PRIMER_TLG,"'ut2015',"),file_get_contents($archivo))));
...
JS
    ...
    function para_proceso_actualizar_instalacion($este,$mas_planas){
        global $esta_es_la_base_en_produccion;
        try{
            Aplicacion_ut2015::salida_enviar_aviso_instalacion($este);
            if($mas_planas){
                foreach(array(
                    new Tablas_planas(),
                ) as $objeto_de_la_base)
...
//                    }
            ...
            //$este->db->ejecutar_sqls(new Sqls(explode('/*OTRA*/',file_get_contents('..\operaciones_ut2015\actualizar_a_ut2015.sql'))));                    
        ...
        return new Proceso_Generico(array(
            ...
            'funcion'=>function(Procesos $este){
                ...
                try{
                    ...
                    $metadatos=$este->nuevo_objeto("Metadatos_ut2015");
...
if(!isset($no_ejecutar_aplicacion)){
    Aplicacion::correr(new Aplicacion_ut2015());
}
?>